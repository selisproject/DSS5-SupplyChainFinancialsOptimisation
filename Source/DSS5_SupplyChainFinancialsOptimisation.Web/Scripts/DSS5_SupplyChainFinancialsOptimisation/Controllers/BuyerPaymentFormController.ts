...
interface Window { scope_BuyerPaymentForm: DSS5_SupplyChainFinancialsOptimisation.Controllers.IBuyerPaymentFormScope; }
namespace DSS5_SupplyChainFinancialsOptimisation.Controllers {
const BuyerPaymentFormConditionalFormattings = [
new Joove.JbRule({
...
condition: async (_parents) => {
        return CLMS.Framework.Number.Equal(() => window["scope_BuyerPaymentForm"].model.Agreement.Id, () => 0);
...
condition: async (_parents) => {
        return Joove.Common.nullSafe<any>(() => window["scope_BuyerPaymentForm"].model.Searched, false);
...
const BuyerPaymentFormDataValidations = [
                                        ];
const BuyerPaymentFormCalculatedExpressions = [
        ];
...
export interface IBuyerPaymentFormScope extends Joove.IWebPageScope {
model:
    DSS5_SupplyChainFinancialsOptimisation.ViewModels.BuyerPaymentForm.BuyerPaymentFormViewModel;
...
class BuyerPaymentFormController {
...
private timeoutDelayLockButton1Clicked:
    ...
    constructor(private $scope: IBuyerPaymentFormScope, $timeout: ng.ITimeoutService) {
        window.scope_BuyerPaymentForm = $scope;
        $scope.$onControlChanged = function (event, newValue, dontMakeDirty) {
            Joove.Core.onChange(event.target, newValue, dontMakeDirty)
        }

        $scope.dehydrate = (context = null) => DSS5_SupplyChainFinancialsOptimisation.ViewModels.BuyerPaymentForm.BuyerPaymentFormViewModel._initializeFrom($scope.model, context);
    ...
    init($scope: IBuyerPaymentFormScope, $timeout: ng.ITimeoutService) {
...
angular.module("Application").controller("BuyerPaymentFormController", ["$scope", "$timeout", BuyerPaymentFormController] as Array<string>);
}
